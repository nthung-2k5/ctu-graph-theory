...
export default function AlgorithmsTab() {
  const { playing, algorithm, setAlgorithm, config, predicateError } =
    useGraphTheory();
  ...
  const items = AvailableAlgorithms.map((algo, index) => ({
    key: index,
    label: (
      <button type="button" onClick={() => setAlgorithm(algo)}>{`${
        index + 1
      }. ${algo.name}`}</button>
    ),
  }));
  ...
  const tabs: TabsProps["items"] = [
    {
      ...
      children: (
        <div className="h-full flex flex-col max-h-full">
          <div>
            <Dropdown trigger={["click"]} disabled={playing} menu={{ items }}>
